...
_Run order: profile (`scripts/inspect_tables.py` local, or `scripts/profile_spark_tables.py` on catalog tables / HDFS / S3) → ideate (backlog) → `build_features.py` → `scripts/signal_panel.py`. Each step produces a real artifact the next consumes._
